...
# the resulting .py programms execute a Randon Forest prediction for a dataset that contains evidently important features from Cell-Painting and Random Forest.
# The Input data for this notebook is stored in 06-* and 07-* and is part of the master thesis of Luis Vollmers
# the output is stored in the _output directory of this subproject 08-* and is a single summarization of the statistical prediction metrics like AUC, Sensitivity and so forth
...
generate_predictions () 
{
	...
	do 
		...
		sed "s/AID/$aid/g" rf_cpfp_AID.py > rf_cpfp_${aid}.py
		# make file executable
		chmod 777 rf_cpfp_${aid}.py
		...
		mv rf_cpfp_${aid}.py ../_trash/
	done

	# move temporary files to the trash
	mv rf_cpfp_AID.py ../_trash/
}
...
summarize_results () 
{
	...
	do
		...
		INP_FILE="rf_cpfp_${aid}_analysis.txt"
	
		# grep the metrics from the individiual analysis files
		# see rf_cpfp_${aid}_analysis.txt for understanding the grep-pattern in detail
