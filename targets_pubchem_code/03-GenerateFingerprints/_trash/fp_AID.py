...
# #### 1. Read the Input
# - input files are being found in step 2 dirctory and are named after their respective AID
# - the original notebook was processing the file cp_AID.csv but that is subject to change
# - for generating the Morgan Fingerprints only the meta data columns are necessary (first 10 columns)

# In[107]:


df = pd.read_csv('../../02-CorrectingAssays/_output/cp_AID.csv')
df = df.iloc[:,:10]
...
df.to_csv('../_output/fp_AID.csv',index=False)

# pd.read_csv('../_output/fp_AID.csv') # only uncomment to check if the output was done correctly
